...
pub mod Set {
    ...
    pub struct SetTree_1<T: Clone + 'static> {
        pub Height: i32,
        pub Key: T,
        pub Left: Option<Rc<Set::SetTree_1<T>>>,
        pub Right: Option<Rc<Set::SetTree_1<T>>>,
    }
    pub fn empty<T: Clone + 'static>() -> Option<Rc<Set::SetTree_1<T>>> {
        None::<Rc<Set::SetTree_1<T>>>
    }
    pub fn isEmpty<T: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<T>>>)
     -> bool {
        s.is_none()
    }
    pub fn mkSetTreeLeaf<T: Clone + 'static>(key: &T)
     -> Option<Rc<Set::SetTree_1<T>>> {
        Some(Rc::from(Set::SetTree_1::<T>{Height: 1i32,
                                          Key: key.clone(),
                                          Left: Set::empty(),
                                          Right: Set::empty(),}))
    }
    pub fn mkSetTreeNode<T: Clone +
                         'static>(key: &T,
                                  left: &Option<Rc<Set::SetTree_1<T>>>,
                                  right: &Option<Rc<Set::SetTree_1<T>>>,
                                  height: &i32)
     -> Option<Rc<Set::SetTree_1<T>>> {
        Some(Rc::from(Set::SetTree_1::<T>{Height: height.clone(),
                                          Key: key.clone(),
                                          Left: left.clone(),
                                          Right: right.clone(),}))
    }
    ...
    pub fn countAux<T: Clone +
                    'static>(s: &Option<Rc<Set::SetTree_1<T>>>, acc: &i32)
     -> i32 {
        match s {
            Some(s_0_0) => {
                let t: Rc<Set::SetTree_1<T>> = s_0_0.clone();
                if t.Height == 1i32 {
                    acc.clone() + 1i32
                } else {
                    Set::countAux(&t.Left,
    ...
    pub fn count<a_: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<a_>>>)
     -> i32 {
        Set::countAux(s, &0i32)
    }
    pub fn height<T: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<T>>>)
     -> i32 {
        match s { Some(s_0_0) => s_0_0.Height, _ => 0i32, }
    }
    pub fn mk<T: Clone +
              'static>(l: &Option<Rc<Set::SetTree_1<T>>>, k: &T,
                       r: &Option<Rc<Set::SetTree_1<T>>>)
     -> Option<Rc<Set::SetTree_1<T>>> {
        {
            let hl: i32 =
                {
                    let s: Option<Rc<Set::SetTree_1<T>>> = l.clone();
                    match &s { Some(s_0_0) => s_0_0.Height, _ => 0i32, }
                };
            let hr: i32 =
                {
                    let s_1: Option<Rc<Set::SetTree_1<T>>> = r.clone();
    ...
    pub fn rebalance<T: Clone +
                     'static>(t1: &Option<Rc<Set::SetTree_1<T>>>, v: &T,
                              t2: &Option<Rc<Set::SetTree_1<T>>>)
     -> Option<Rc<Set::SetTree_1<T>>> {
        {
            let t1h: i32 =
                {
                    let s: Option<Rc<Set::SetTree_1<T>>> = t1.clone();
                    match &s { Some(s_0_0) => s_0_0.Height, _ => 0i32, }
                };
            let t2h: i32 =
                {
                    let s_1: Option<Rc<Set::SetTree_1<T>>> = t2.clone();
                    match &s_1 { Some(s_1_0_0) => s_1_0_0.Height, _ => 0i32, }
                };
            if t2h > t1h + 2i32 {
                {
                    let t2_0027: Rc<Set::SetTree_1<T>> =
                        Option_::getValue(t2).clone();
                    if {
                           let s_2: Option<Rc<Set::SetTree_1<T>>> =
                               t2_0027.Left.clone();
                           match &s_2 {
                ...
                if t1h > t2h + 2i32 {
                    {
                        let t1_0027: Rc<Set::SetTree_1<T>> =
                            Option_::getValue(t1).clone();
                        if {
                               let s_3: Option<Rc<Set::SetTree_1<T>>> =
                                   t1_0027.Right.clone();
                               match &s_3 {
    ...
    pub fn add<T: PartialOrd + Clone +
               'static>(k: &T, s: &Option<Rc<Set::SetTree_1<T>>>)
     -> Option<Rc<Set::SetTree_1<T>>> {
        match s {
            Some(s_0_0) =>
            {
    ...
    pub fn balance<T: PartialOrd + Clone +
                   'static>(s1: &Option<Rc<Set::SetTree_1<T>>>, k: &T,
                            s2: &Option<Rc<Set::SetTree_1<T>>>)
     -> Option<Rc<Set::SetTree_1<T>>> {
        match s1 {
            Some(s1_0_0) =>
            {
                let t1: Rc<Set::SetTree_1<T>> = s1_0_0.clone();
                match s2 {
                    Some(s2_0_0) =>
                    {
                        let t2: Rc<Set::SetTree_1<T>> = s2_0_0.clone();
                        if t1.Height == 1i32 {
    ...
    pub fn split<T: PartialOrd + Clone +
                 'static>(pivot: &T, s: &Option<Rc<Set::SetTree_1<T>>>)
     -> (Option<Rc<Set::SetTree_1<T>>>, bool, Option<Rc<Set::SetTree_1<T>>>) {
        match s {
            Some(s_0_0) => {
                let t: Rc<Set::SetTree_1<T>> = s_0_0.clone();
                if t.Height == 1i32 {
                    ...
                    if c < 0i32 {
                        (s.clone(), false, Set::empty())
                    } else {
                        if c == 0i32 {
                            (Set::empty(), true, Set::empty())
                        } else { (Set::empty(), false, s.clone()) }
                    }
                } else {
                    let c_1: i32 = Util::compare(pivot, &t.Key);
                    if c_1 < 0i32 {
                        let patternInput:
                                (Option<Rc<Set::SetTree_1<T>>>, bool,
                                 Option<Rc<Set::SetTree_1<T>>>) =
                            Set::split(pivot, &t.Left);
                        (patternInput.0.clone(), patternInput.1.clone(),
                         Set::balance(&patternInput.2.clone(), &t.Key,
                                      &t.Right))
                    } else {
                        if c_1 == 0i32 {
            ...
            _ => (Set::empty(), false, Set::empty()),
        }
    }
    pub fn spliceOutSuccessor<T: Clone +
                              'static>(s: &Option<Rc<Set::SetTree_1<T>>>)
     -> (T, Option<Rc<Set::SetTree_1<T>>>) {
        match s {
            Some(s_0_0) => {
                let t: Rc<Set::SetTree_1<T>> = s_0_0.clone();
                if t.Height == 1i32 {
                    (t.Key.clone(), Set::empty())
                } else {
                    if t.Left.is_none() {
                        (t.Key.clone(), t.Right.clone())
                    } else {
                        let patternInput: (T, Option<Rc<Set::SetTree_1<T>>>) =
                            Set::spliceOutSuccessor(&t.Left);
                        (patternInput.0.clone(),
                         Set::mk(&patternInput.1.clone(), &t.Key, &t.Right))
                    }
                }
            }
            _ =>
    ...
    pub fn foldBack<T: Clone + 'static, a_: Clone +
                    'static>(f: &Rc<impl Fn(&T, &a_) -> (a_) + 'static>,
                             s: &Option<Rc<Set::SetTree_1<T>>>, x: &a_)
     -> a_ {
        match s {
            Some(s_0_0) =>
            {
    ...
    pub fn fold<a_: Clone + 'static, T: Clone +
                'static>(f: &Rc<impl Fn(&a_, &T) -> (a_) + 'static>, x: &a_,
                         s: &Option<Rc<Set::SetTree_1<T>>>) -> a_ {
        match s {
            Some(s_0_0) =>
            {
                let t: Rc<Set::SetTree_1<T>> = s_0_0.clone();
    ...
    pub fn map<T: Clone + 'static, a_: PartialOrd + Clone +
               'static>(mapping: &Rc<impl Fn(&T) -> (a_) + 'static>,
                        s: &Option<Rc<Set::SetTree_1<T>>>)
     -> Option<Rc<Set::SetTree_1<a_>>> {
        Set::fold(&Rc::from({
                                let mapping = mapping.clone();
                                move
                                    |acc: &Option<Rc<Set::SetTree_1<a_>>>,
                                     k: &T| Set::add(&mapping(k), acc)
                            }), &Set::empty(), s)
    }
    pub fn forAll<T: Clone +
                  'static>(f: &Rc<impl Fn(&T) -> (bool) + 'static>,
                           s: &Option<Rc<Set::SetTree_1<T>>>) -> bool {
        match s {
            Some(s_0_0) => {
                let t: Rc<Set::SetTree_1<T>> = s_0_0.clone();
                if t.Height == 1i32 {
    ...
    pub fn isSubset<a_: PartialOrd + Clone +
                    'static>(a: &Option<Rc<Set::SetTree_1<a_>>>,
                             b: &Option<Rc<Set::SetTree_1<a_>>>) -> bool {
        Set::forAll(&Rc::from({
                                  let b = b.clone();
    ...
    pub fn diffAux<T: PartialOrd + Clone +
     ...
     -> Option<Rc<Set::SetTree_1<T>>> {
        match acc {
            Some(acc_0_0) =>
            {
                let _acc: Rc<Set::SetTree_1<T>> = acc_0_0.clone();
                match s {
                    Some(s_0_0) =>
                    {
                        let t: Rc<Set::SetTree_1<T>> = s_0_0.clone();
    ...
    pub fn r#union<T: PartialOrd + Clone +
                   'static>(s1: &Option<Rc<Set::SetTree_1<T>>>,
                            s2: &Option<Rc<Set::SetTree_1<T>>>)
     -> Option<Rc<Set::SetTree_1<T>>> {
        match s1 {
            Some(s1_0_0) =>
    ...
    pub fn unionMany<T: PartialOrd + Clone +
                     'static>(sets:
                                  &Rc<dyn Interfaces::IEnumerable_1<Option<Rc<Set::SetTree_1<T>>>>>)
     -> Option<Rc<Set::SetTree_1<T>>> {
        Seq::fold(&Rc::from(move
                                |s1: &Option<Rc<Set::SetTree_1<T>>>,
                                 s2: &Option<Rc<Set::SetTree_1<T>>>|
                                Set::r#union(s1, s2)), &Set::empty(), sets)
    }
    pub fn intersectionAux<T: PartialOrd + Clone +
                           'static>(b: &Option<Rc<Set::SetTree_1<T>>>,
                                    s: &Option<Rc<Set::SetTree_1<T>>>,
                                    acc: &Option<Rc<Set::SetTree_1<T>>>)
     -> Option<Rc<Set::SetTree_1<T>>> {
        match s {
            Some(s_0_0) =>
            {
                ...
                if t.Height == 1i32 {
                    ...
                    {
                        let acc_1: Option<Rc<Set::SetTree_1<T>>> =
    ...
    pub fn intersectMany<T: PartialOrd + Clone +
     ...
     -> Option<Rc<Set::SetTree_1<T>>> {
        Seq::reduce(&Rc::from(move
                                  |a: &Option<Rc<Set::SetTree_1<T>>>,
                                   b: &Option<Rc<Set::SetTree_1<T>>>|
                                  Set::intersect(a, b)), sets)
    }
    pub fn partition1<a_: PartialOrd + Clone +
                      'static>(f: &Rc<impl Fn(&a_) -> (bool) + 'static>,
                               k: &a_, acc1: &Option<Rc<Set::SetTree_1<a_>>>,
                               acc2: &Option<Rc<Set::SetTree_1<a_>>>)
     -> (Option<Rc<Set::SetTree_1<a_>>>, Option<Rc<Set::SetTree_1<a_>>>) {
        if f(k) {
            (Set::add(k, acc1), acc2.clone())
        } else { (acc1.clone(), Set::add(k, acc2)) }
    }
    pub fn partitionAux<T: PartialOrd + Clone +
                        'static>(f: &Rc<impl Fn(&T) -> (bool) + 'static>,
                                 s: &Option<Rc<Set::SetTree_1<T>>>,
                                 acc_0: &Option<Rc<Set::SetTree_1<T>>>,
                                 acc_1: &Option<Rc<Set::SetTree_1<T>>>)
     -> (Option<Rc<Set::SetTree_1<T>>>, Option<Rc<Set::SetTree_1<T>>>) {
        let acc:
                (Option<Rc<Set::SetTree_1<T>>>,
                 Option<Rc<Set::SetTree_1<T>>>) =
        ...
        match s {
            Some(s_0_0) => {
                let t: Rc<Set::SetTree_1<T>> = s_0_0.clone();
                if t.Height == 1i32 {
                    Set::partition1(f, &t.Key, &acc.0.clone(), &acc.1.clone())
                } else {
                    let acc_2:
                            (Option<Rc<Set::SetTree_1<T>>>,
                             Option<Rc<Set::SetTree_1<T>>>) =
    ...
    pub fn minElement<a_: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<a_>>>)
     -> a_ {
        {
    ...
    pub fn maxElement<a_: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<a_>>>)
     -> a_ {
        {
    ...
    pub struct SetIterator_1<T: PartialOrd + Clone + 'static> {
        pub stack: MutCell<List_1<Option<Rc<Set::SetTree_1<T>>>>>,
        pub started: MutCell<bool>,
    }
    pub fn collapseLHS<T: Clone +
                       'static>(stack: &List_1<Option<Rc<Set::SetTree_1<T>>>>)
     -> List_1<Option<Rc<Set::SetTree_1<T>>>> {
        if !List::isEmpty(stack) {
            let s: Option<Rc<Set::SetTree_1<T>>> = List::head(stack).clone();
            let rest: List_1<Option<Rc<Set::SetTree_1<T>>>> =
        ...
        } else { List::empty::<Option<Rc<Set::SetTree_1<T>>>>() }
    }
    pub fn mkIterator<a_: PartialOrd + Clone +
                      'static>(s: &Option<Rc<Set::SetTree_1<a_>>>)
    ...
    pub fn current<T: PartialOrd + Clone +
        ...
        if i.started.get() {
            {
                let matchValue: List_1<Option<Rc<Set::SetTree_1<T>>>> =
                    i.stack.get().clone();
                if !List::isEmpty(&matchValue) {
                    if List::head(&matchValue).is_some() {
                        let k: Rc<Set::SetTree_1<T>> =
    ...
    pub fn choose<a_: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<a_>>>)
     -> a_ {
        Set::minElement(s)
    }
    ...
    pub fn toArray<T: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<T>>>)
     -> Rc<MutCell<Vec<T>>> {
        {
            let res: Rc<MutCell<Vec<T>>> =
                Native::arrayWithCapacity::<T>(&Set::count(s));
            Set::iterate(&Rc::from({
                                       let res = res.clone();
                                       move |x: &T|
    ...
    pub fn toList<T: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<T>>>)
     -> List_1<T> {
        Set::foldBack(&Rc::from(move |k: &T, acc: &List_1<T>|
                                    List::cons(k, acc)), s,
                      &List::empty::<T>())
    }
    pub fn toSeq<T: Clone + 'static>(s: &Option<Rc<Set::SetTree_1<T>>>)
     -> Rc<dyn Interfaces::IEnumerable_1<T>> {
        Seq::delay(&Rc::from({
                                 let s = s.clone();
                                 move || Seq::ofArray(&Set::toArray(&s))
                             }))
    }
    pub fn compareTo<T: PartialOrd + Clone +
                     'static>(s1: &Option<Rc<Set::SetTree_1<T>>>,
                              s2: &Option<Rc<Set::SetTree_1<T>>>) -> i32 {
        Util::compare(&Set::toArray(s1), &Set::toArray(s2))
    }
    pub fn equalsTo<T: Eq + core::hash::Hash + Clone +
                    'static>(s1: &Option<Rc<Set::SetTree_1<T>>>,
                             s2: &Option<Rc<Set::SetTree_1<T>>>) -> bool {
        Set::toArray(s1) == Set::toArray(s2)
    }
    ...
    pub fn ofSeq<a_: PartialOrd + Clone +
     ...
     -> Option<Rc<Set::SetTree_1<a_>>> {
        Seq::fold(&Rc::from(move
                                |acc: &Option<Rc<Set::SetTree_1<a_>>>, k: &a_|
                                Set::add(k, acc)), &Set::empty(), xs)
    }
}
